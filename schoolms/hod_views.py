...
from core.models import Course, Session_Year, CustomUser, Student, Staff, Subject, Staff_Notification, Staff_Leave, Staff_Feedback, Student_Notification, Student_Feedback, Student_Leave, Attendance, Attendance_Report
...
def DELETE_STUDENT(request, admin):
    ...
    student.delete()
    messages.success(request, 'Student Was Succesfully Deleted!')
    return redirect('view_student')
...
def ADD_STAFF(request):
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(username=username).exists():
            ...
            staff.save()
            messages.success(request, 'Staff Was Successfully Added!')
            return redirect('add_staff')
...
def DELETE_STAFF(request, admin):
    ...
    staff.delete()
    messages.success(request, 'Staff Was Succesfully Deleted!')
    return redirect('view_staff')
...
def ADD_SUBJECT(request):
    ...
    if request.method == "POST":
        ...
        subject.save()
        messages.success(request, 'Subject Was Successfully Added!')
...
def DELETE_SUBJECT(request, id):
    ...
    subject.delete()
    messages.success(request, 'Subject Was Successfully Deleted!')
    return redirect('view_subject')
...
def ADD_SESSION(request):
    if request.method == "POST":
        ...
        session.save()
        messages.success(request, 'Session Was Successfully Created!')
...
def DELETE_SESSION(request, id):
    ...
    session.delete()
    messages.success(request, 'Session Was Successfully Deleted!')
    return redirect('view_session')

@login_required(login_url='/')
def STAFF_SEND_NOTE(request):
    staff = Staff.objects.all()
    ...
    return render(request, 'hod/staff_send_note.html', context)

@login_required(login_url='/')
def SAVE_STAFF_NOTE(request):
    if request.method == "POST":
        staff_id = request.POST.get('staff_id')
        ...
        notifications.save()
        messages.success(request, 'Notification Was Successfully Sent!')
        return redirect('staff_send_note')
...
def APPROVE_LEAVE(request, id):
    ...
    leave.status = 1
    leave.save()
    messages.success(request, 'Leave Was Successfully Approved!')
...
def DISAPPROVE_LEAVE(request, id):
    ...
    leave.status = 2
    leave.save()
    messages.error(request, 'Leave Was Not Approved!')
    return redirect('staff_leave_view')
...
def STAFF_FEEDBACK_SAVE(request):
    if request.method == "POST":
        ...
        feedback.status = 1
        feedback.save()
        messages.success(request, 'Reply Was Successfully Sent!')
...
def STUDENT_SAVE_NOTIFICATION(request):
    if request.method == "POST":
        ...
        messages.success(request, 'Student Notification Was Successfully Sent!')
